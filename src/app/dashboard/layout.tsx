    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            if (decoded) {
                ...
                if (role) {
                    setRolActual(role);
                    if (role === 'TENANT' && pathname === '/dashboard') {
                        router.push('/dashboard/inmuebles');
                    }
                    if (role === 'TECHNICIAN' && pathname === '/dashboard') {
                ...
                <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
                    ...
                    {rolActual !== 'TECHNICIAN' && (
                        <Link href="/dashboard/inmuebles" className={classLink('/dashboard/inmuebles')}>
                            ...
                            <span>{rolActual === 'TENANT' ? 'Mi Propiedad' : 'Inmuebles'}</span>
                        </Link>
                    )}
                    {rolActual !== 'TENANT' && rolActual !== 'TECHNICIAN' && (
                    ...
                    {rolActual !== 'TECHNICIAN' && (
                        <Link href="/dashboard/inventarios" className={classLink('/dashboard/inventarios')}>
                            ...
                            <span>{rolActual === 'TENANT' ? 'Inventario' : 'Inventarios'}</span>
                        </Link>
                    )}
                    ...
                    <Link href="/dashboard/tickets" className={classLink('/dashboard/tickets')}>
                        <div className="relative flex items-center w-full justify-between">
                            <div className="flex items-center space-x-3">
                                ...
                                <span>{rolActual === 'TECHNICIAN' ? 'Mis Tickets' : 'Tickets'}</span>
                            </div>
                        </div>
                    </Link>
