...
export default function UsuariosPage() {
    ...
    const [filter, setFilter] = useState<'all' | 'ADMIN' | 'ASSISTANT' | 'TECHNICIAN'>('all');
    ...
    const [editForm, setEditForm] = useState({
        ...
        document_type: '',
        document_number: '',
        role: 'ASSISTANT' as 'ADMIN' | 'ASSISTANT' | 'TENANT' | 'TECHNICIAN',
        is_active: true,
        password: ''
    });
                    ...
                    try {
                        if (!editForm.is_active) {
                            ...
                            if (deactivateRes.status === 409) {
                                ...
                                const confirmed = await showConfirm(msg, "Desactivar Usuario");
                                if (confirmed) {
    ...
    const stats = {
        ...
        tecnicos: staff.filter(u => u.role === 'TECHNICIAN').length,
    };
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    { label: 'Total Staff', value: stats.total, color: 'bg-indigo-600 dark:bg-indigo-500 text-white shadow-indigo-600/20', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z' },
                    { label: 'Administradores', value: stats.admins, color: 'bg-purple-600 dark:bg-purple-500 text-white shadow-purple-600/20', icon: 'M9 12l2 2 4-4m5.618-4.016A3.323 3.323 0 0010.603 2L12 10l-2.293 2.293a1 1 0 000 1.414L12 16l1.397-8.382a3.323 3.323 0 004.618 4.016z' },
                    { label: 'Asistentes', value: stats.asistentes, color: 'bg-amber-600 dark:bg-amber-500 text-white shadow-amber-600/20', icon: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z' },
                    { label: 'Técnicos', value: stats.tecnicos, color: 'bg-teal-600 dark:bg-teal-500 text-white shadow-teal-600/20', icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z' },
                ].map((s, i) => (
                    <div key={i} className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center gap-5 hover:shadow-md transition-all group/card">
                        <div className={`w-14 h-14 ${s.color} rounded-2xl flex items-center justify-center shrink-0 shadow-lg group-hover/card:scale-110 transition-transform`}>
                                ...
                                <tr key={u.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors group">
                                    ...
                                    <td className="px-8 py-5">
                                        <span className={`px-2.5 py-1 rounded text-[9px] font-black uppercase tracking-tight ${
                                            ...
                                            u.role === 'ASSISTANT' ? 'bg-orange-100 text-orange-700' : 'bg-teal-100 text-teal-700'
                                        }`}>
                                            {u.role_display}
                                        </span>
                                    </td>
            ...
            >
                {editingUsuario && (
                    <form onSubmit={handleEditSubmit} className="space-y-5">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div className="space-y-1.5">
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-slate-900 dark:focus:ring-rose-500 dark:text-white transition text-xs shadow-sm"
                                />
                            </div>
                            <div className="space-y-1.5">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div className="space-y-1.5">
                                <label className="text-[11px] font-bold tracking-widest text-slate-500 dark:text-slate-400 uppercase">Teléfono</label>
                                <input
                                    type="text"
                            ...
                            <div className="space-y-1.5">
                                ...
                                <select
                                    ...
                                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-slate-900 dark:focus:ring-rose-500 dark:text-white transition text-xs shadow-sm cursor-pointer"
                                >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div className="space-y-1.5">
                                <label className="text-[11px] font-bold tracking-widest text-slate-500 dark:text-slate-400 uppercase">Estado de la cuenta</label>
                                <select
                                    disabled={isCurrentUser(editingUsuario.email)}
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[11px] font-bold tracking-widest text-slate-500 dark:text-slate-400 uppercase">Nueva Contraseña (Opcional)</label>
                                <input
                                    type="password"
                        ...
                        <div className="pt-5 mt-6 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3">
                            <button
                                type="button"
                                onClick={() => setEditingUsuario(null)}
                                className="px-6 py-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-xl transition"
                            >
                                Cancelar
                            </button>
                            <button
                                type="submit"
                                disabled={submitting}
                                className="px-6 py-3 bg-slate-900 hover:bg-slate-800 dark:bg-rose-600 dark:hover:bg-rose-500 disabled:bg-rose-400 text-white text-xs font-bold rounded-xl transition shadow-md"
                            >
